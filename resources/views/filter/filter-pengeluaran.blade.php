<div class="col-md-3">
	...
	<select class="form-control" id="f_id_pengeluaran" name="f_id_pengeluaran">
		<option value="">-- cari pengeluaran --</option>
		@foreach($pengeluaran as $key => $value)
			<option value="{{ $value->id_pengeluaran }}">{{ $value->kode_pengeluaran }}</option>
		@endforeach
	</select>
</div>
...
<div class="col-md-12 text-right mt-1">
    ...
    <div class="dropdown d-inline-block">
		...
		<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
			<a class="dropdown-item" href="{{ route('cetakallpengeluaran', [
                'dr_tgl' => $filter['dr_tgl'],
                'sp_tgl' => $filter['sp_tgl'],
                ]) }}" target="_blank" rel="nofollow">Cetak Pengeluaran</a>
		</div>
	</div>
</div>
