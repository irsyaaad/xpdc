...
<form method="GET" action="{{ url("cashflowdetail") }}" enctype="multipart/form-data" id="form-select">
    @csrf
    <div class="row">
        <div class="col-md-4">
                    ...
                    @if (isset($cashout[$value->id_ac]) || isset($cashin[$value->id_ac]))
                        ...
                        @if(isset($cashin[$value->id_ac]))
                            ...
                            @foreach($cashin[$value->id_ac] as $key2 => $value2)
                                @foreach ($filter_ac as $acnya)
                                    @isset($cashin[$value->id_ac][$acnya->id_ac])
                                        <tr class="tr-bold">
                                            <td colspan=7>{{ strtoupper($acnya->nama) }}</td>
                                        </tr> 
                                        @php 
                                            $sub_total_ac   = 0;
                                            $nomer          = 1;
                                        @endphp
                                        @foreach ($cashin[$value->id_ac][$acnya->id_ac] as $item)
                                            <tr>
                                                <td>{{ $nomer++ }}</td>
                                                <td>{{ dateindo($item->tgl) }}</td>
                                                <td>{{ $item->id_detail }}</td>
                                                <td>{{ $item->keterangan }}</td>
                                                <td class="text-right">{{ number_format($item->nominal, 0, ',', '.') }}</td>
                                                <td class="text-right">0</td>
                                                    @php $sub_total_ac += $item->nominal @endphp
                                                <td>{{ strtoupper($item->nm_user) }}</td>
                                            </tr>
                                        @endforeach
                                        <tr class="tr-bold">
                                            <td colspan=4 class="text-right">Sub Total {{ $acnya->nama }}</td>
