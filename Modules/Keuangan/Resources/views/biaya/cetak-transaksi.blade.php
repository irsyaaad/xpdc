...
<div id="print-kan">
    ...
    <div class="container" style="margin-top:20px">
        <div>
            <p class="text-center"><b>Bukti Kas / Bank Keluar</b></p>
        </div>
        <table class="atas" width="100%">
            <tr>
                <td width="15%">No. Transaksi</td>
                <td width="2%"> : </td>
                <td>{{ $data->kode_dm . '-' . $data->id_biaya }}</td>
            </tr>
            <tr>
                <td>Tgl. Bayar</td>
                <td> : </td>
                <td>
                    @if (isset($data->tgl_bayar))
                        {{ daydate($data->tgl_bayar) . ', ' . dateindo($data->tgl_bayar) }}
                    @endif
                </td>
            </tr>
            <tr>
                ...
                <td> : </td>
                <td>
                    @if (isset($data->kredit->nama))
                        {{ strtoupper('( ' . $data->kredit->id_ac . ' )   ' . $data->kredit->nama) }}
                    @endif
                </td>
            </tr>
            <tr>
                <td>Penerima</td>
                <td> : </td>
                <td>{{ strtoupper($data->user->nm_user) }}</td>
            </tr>
        </table>
        <br>
            ...
            <tbody>
                <tr>
                    ...
                    <td class="t text-left">
                        @if (isset($data->debet->id_ac))
                            {{ strtoupper($data->debet->nama) }}
                        @endif
                        <br><br>{{ $data->info }}
                    </td>
                    <td class="t">1</td>
                    <td class="t">{{ number_format($data->n_bayar, 0, ',', '.') }}</td>
                    <td class="t">{{ number_format($data->n_bayar, 0, ',', '.') }}</td>
                </tr>
                <tr>
                    <td colspan="4" class="t text-center">Total</td>
                    <td colspan="1" class="t text-center">Rp. {{ number_format($data->n_bayar, 0, ',', '.') }}</td>
                </tr>
            </tbody>
        </table>
    </div>
