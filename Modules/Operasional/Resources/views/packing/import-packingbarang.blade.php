...
<form method="POST" action="{{ Request::segment(1) }}" enctype="multipart/form-data">
    ...
    <div class="row" style="margin-top: -1%">
        ...
        <div class="form-group col-md-3">
            ...
            <input class="form-control m-input m-input--square" value="@if(isset($data->perush_asal->nm_perush)){{ $data->perush_asal->nm_perush }}@else {{ old("nm_pelanggan") }} @endif" readonly type="text" name="nm_pelanggan" id="nm_pelanggan" />
        </div>
        ...
        <div class="col-md-12">
            <div class="text-right">
                ...
                <button type="button" onclick="goPopUp()" class="btn btn-sm btn-primary"><i class="fa fa-plus"> </i> Tambah Barang</button>
                ...
                <tbody> 
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <a class="btn btn-sm btn-warning" href="#" onclick="gOEdit('{{ $value->id_detail }}')"><i class="fa fa-edit"></i></a>
                            <a class="btn btn-sm btn-danger" href="#"  onclick="CheckDetailDelete('{{ $value->id_detail }}')"><i class="fa fa-times"></i></a>
                            @endif
                        </td>
                    </tr>
                    @endforeach
...
<div class="modal fade" id="modal-import" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <form method="POST" action="{{ url(Request::segment(1)."/doimport/".Request::segment(3)) }}" enctype="multipart/form-data" id="form-import">
    ...
    <div class="modal-dialog modal-dialog-centered modal-md" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <form method="POST" action="#" enctype="multipart/form-data" name="form-delete" id="form-delete" >
                    {{ method_field("DELETE") }}
                    @csrf
                    <button type="submit" class="btn btn-success"><i class="fa fa-check"> </i> Ya</button>
                    <button type="button" class="btn btn-danger" onclick="goClose()" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"> <i class="fa fa-times"> </i> Tidak</span></button>
                </form>
            </div>
        </div>
    </div>
...
<script> 
    ...
    $("#id_jenis_packing").change(function() {
        goHitungVolume();
        getTotal();
    });
    
    function goHitungVolume(){
        var panjang = parseFloat($("#panjang").val());
        var lebar = parseFloat($("#lebar").val());
        var tinggi = parseFloat($("#tinggi").val());
        var volume = parseFloat(panjang*lebar*tinggi);
    ...
    function getTarif(){
        var volume = parseFloat($("#n_volume").val());
        
        $.ajax({
            type: "POST",
            data: {"volume" : volume, "id_jenis_packing" : $("#id_jenis_packing").val(), "_token": "{{ csrf_token() }}" },
            dataType: "json",
            url: "{{ url('tarifpacking/gettarifpacking') }}",
    ...
    function gOEdit(id){
        ...
        $('#id_tipe_kirim').prop('disabled', true);
    }
    ...
    function CheckDetailDelete(id){
        $("#form-delete").attr("action", "{{ url(Request::segment(1)) }}/deletedetail/"+id);
