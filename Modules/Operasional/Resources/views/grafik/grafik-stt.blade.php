<form method="POST" action="{{ url('operasional/filter') }}" id="form-share" name="form-share">
    @csrf
<div class="row">
<div class="col-md-3">
	...
	<div class="m-form__group m-form__group--inline">
		...
		<label style="font-weight: bold;">
			...
			<select class="form-control" id="status" name="status">
                <option value="0">-- Pilih Status Stt --</option>
            </select>
		</div>
	</div>
	<div class="d-md-none m--margin-bottom-10"></div>
...
<br>
<canvas id="stt" class="col-md-11" style="padding-left:10%"></canvas>
<script>
    
    @if(Session('bulan') != null)
		$("#filter").val('{{ Session('bulan') }}');
	@endif
    ...
    $.ajax({
        ...
        success: function (response) {
            $.each(response, function (key, value) {
                $("#status").append('<option value=' + value.kode + '>' + value.value + '</option>');
            });
    ...
    //bar
    var data = <?php echo json_encode( $stt ) ?>;
    var temp = [];
    ...
    console.log(temp);
    var ctxB = document.getElementById("stt").getContext('2d');
    var myBarChart = new Chart(ctxB, {
        type: 'bar',
        data: {
            labels: tgl,
            datasets: [{
                label: 'Jumlah STT / Tanggal Masuk',
                data: temp,
                backgroundColor: [
                    'rgba(255, 99, 132, 0.2)',
                    'rgba(54, 162, 235, 0.2)',
