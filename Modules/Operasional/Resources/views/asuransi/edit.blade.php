...
<hr>
<form class="m-form m-form--fit m-form--label-align-right" method="POST" action="{{ url('asuransistt', $data->id_asuransi) }}" enctype="multipart/form-data">
    {{ method_field("PUT") }}
    @csrf
    ...
    <div class="row form-group m-form__group">
        ...
        <div class="col-md-6">
            ...
            <input type="text" name="nm_pengirim" id="nm_pengirim" class="form-control" placeholder="Ex : Ayu Mutia" value="@if (isset($data->nm_pengirim)){{$data->nm_pengirim}}@endif">
    ...
    <div class="row form-group m-form__group">
        <div class="col-md-6">
            ...
            <input type="date" name="tgl_berangkat" id="tgl_berangkat" class="form-control" value="@if (isset($data->tgl_berangkat)){{$data->tgl_berangkat}}@endif">
        ...
        <div class="col-md-6">
            ...
            <input type="date" name="tgl_sampai" id="tgl_sampai" class="form-control" value="@if (isset($data->tgl_sampai)){{$data->tgl_sampai}}@endif">
    ...
    <div class="row form-group m-form__group">
        ...
        <div class="col-md-6">
            ...
            <input type="number" name="qty" id="qty" class="form-control" value="@if (isset($data->qty)){{$data->qty}}@endif">
    ...
    <div class="row form-group m-form__group">
        ...
        <div class="col-md-6">
            ...
            <input type="number" class="form-control m-input m-input--square" id="n_pertanggungan" name="n_pertanggungan" maxlength="100" step="any" value="@if(isset($data->harga_pertanggungan)){{ $data->harga_pertanggungan }}@else{{ old("n_pertanggungan") }}@endif" required="required">
    ...
    <div class="row form-group m-form__group">
        <div class="col">
            ...
            <input type="number" class="form-control m-input m-input--square" id="nominal_beli" name="nominal_beli" maxlength="100" step="any" value="@if(isset($data->premi)){{ $data->premi }}@else{{ old("nominal_beli") }}@endif" required="required" readonly>
        </div>
        <div class="col">
            <label for="nominal">
                <b>Harga Jual</b> <span class="span-required"> *</span>
            </label>
            <input type="number" class="form-control m-input m-input--square" id="nominal_jual" name="nominal_jual" maxlength="100" step="any" value="@if(isset($data->nominal)){{ $data->nominal }}@else{{ old("nominal_jual") }}@endif" required="required" readonly>
        </div>
    </div>
...
<script>
    @if(isset($data->pelanggan->id_perush))
    $("#id_pelanggan").empty();
    $("#id_pelanggan").append('<option value=' + {{ $data->pelanggan->id_perush }} + '>'+"{{ strtoupper($data->pelanggan->nm_perush) }}"+'</option>');
    @endif

    @if(isset($data->id_stt))
    $("#stt_asur").empty();
    $("#stt_asur").append("<option value={{ $data->id_stt }}>{{ strtoupper($data->id_stt) }}</option>");
    @endif

    @if(isset($data->asal->id_wil))
    $("#id_asal").empty();
    $("#id_asal").append('<option value=' + {{ $data->asal->id_wil }} + '>'+"{{ strtoupper($data->asal->nama_wil) }}"+'</option>');
    @endif

    @if(isset($data->tujuan->id_wil))
    $("#id_tujuan").empty();
    $("#id_tujuan").append('<option value=' + {{ $data->tujuan->id_wil }} + '>'+"{{ strtoupper($data->tujuan->nama_wil) }}"+'</option>');
    @endif

    @if(isset($data->tipebarang->id_tipe_kirim))
    $("#id_tipe_barang").empty();
    $("#id_tipe_barang").append("<option value={{ $data->tipebarang->id_tipe_kirim }}>{{ strtoupper($data->tipebarang->nm_tipe_kirim) }}</option>");
    @endif
