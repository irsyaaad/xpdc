...
<div class="col-md-12 text-right" id="tombol" style="margin-top:20px">
    ...
    <div class="container" style="margin-top:10px">
        <table width="100%" >
            <tr>
                ...
                <td width="25%" colspan="2" style="border: 1px solid black;">@if(isset($data->tgl_masuk)){{ daydate($data->tgl_masuk).", ".dateindo($data->tgl_masuk) }}@endif</td>
            </tr>
            <tr>
                <td colspan="2"><b>SURAT TANDA TERIMA TITIPAN No.</b></td>
                <td>@if(isset($data->kode_stt)){{ $data->kode_stt }}@endif</td>
                <td colspan="2" style="border: 1px solid black;">TANGGAL OUT</td>
                <td colspan="2" style="border: 1px solid black;">@if(isset($data->tgl_keluar)){{ daydate($data->tgl_keluar).", ".dateindo($data->tgl_keluar) }}@endif</td>
            </tr>
            <tr style="border-top: 1px solid black; border-left: 1px solid black;">
                <td width="10%" >Kepada</td>
                <td colspan="2">: &nbsp {{ strtoupper($data->penerima_nm) }}</td>
            ...
            <tr style="border-left: 1px solid black;" height="50px">
                ...
                <td colspan="2" class="almt">: &nbsp {{ strtoupper($data->penerima_alm) }} - {{ $data->penerima_kodepos }}, @if(isset($data->tujuan->nama_wil)){{ $data->tujuan->nama_wil }}@endif</td>
                <td colspan="4" rowspan="2" class="text-center" style="border: 1px solid black;padding-top:10px; font-size:20px;"><b>@if(isset($data->tipekirim->nm_tipe_kirim)){{ $data->tipekirim->nm_tipe_kirim }}
                                    @endif</b></td>
            </tr>
            <tr style="border-left: 1px solid black;">
                <td >No Telpn</td>
                <td colspan="2">: &nbsp {{ $data->penerima_telp }}</td>
            </tr>
            <tr style="border-top: 1px solid black; border-left: 1px solid black;">
                <td width="10%">Pengirim</td>
                <td colspan="2">: &nbsp {{ strtoupper($data->pengirim_nm) }}</td>
            ...
            <tr style="border-left: 1px solid black;" height="50px">
                <td class="almt">Alamat</td>
                <td colspan="2" class="almt">: &nbsp {{ strtoupper($data->pengirim_alm) }} - {{ $data->pengirim_kodepos }}, @if(isset($data->asal->nama_wil)){{ $data->asal->nama_wil }} @endif</td>
                <td colspan="2" rowspan="2" class="text-center" style="border: 1px solid black; padding-top:10px"><b>@if(isset($data->cara_kemas)){{ strtoupper($data->cara_kemas) }}@endif</b></td>
                @php
                    $info_kirim = '';
                    if (isset($data->info_kirim)) {
                        $total = strlen($data->info_kirim);
                        if ($total > 15) {
                            $info_kirim = substr($data->info_kirim, 0, 15) . '[...]';
                        } else {
                            $info_kirim = $data->info_kirim;
                        }
                    }
                @endphp
                <td colspan="2" rowspan="2" class="text-center" style="border: 1px solid black; padding-top:10px"><b>{{ strtoupper($info_kirim) }}</b></td>
            </tr>
            <tr style="border-left: 1px solid black;">
                <td >No Telpn</td>
                <td colspan="2">: &nbsp {{ $data->pengirim_telp }}</td>
            </tr>
            <tr height="30px">
    ...
    <div class=container>
        <div class="row">
            ...
            <div class="col">
                {{ strtoupper($data->penerima_nm) }}
            </div>
            <div class="col">
                {{ strtoupper($data->pengirim_nm) }}
            </div>
            <div class="col">
